...
class GameApp:
    def __init__(self, root):
        ...
        self.root.configure(bg="#FFB6B6")  # Fondo rosa claro
        ...
        self.main_frame = tk.Frame(root, bg="#FFB6B6", padx=20, pady=20)
        self.main_frame.pack(fill=tk.BOTH, expand=True)

        # Frame para la conexión Arduino
        self.arduino_frame = tk.Frame(self.main_frame, bg="#FFB6B6", pady=10)
        self.arduino_frame.pack(fill=tk.X)

        # Dropdown para puertos COM
        self.port_label = tk.Label(
            self.arduino_frame, text="Puerto Arduino:", bg="#FFB6B6", font=("Arial", 10))
        ...
        self.title_frame = tk.Frame(
            self.main_frame, bg="#FFEF99", padx=10, pady=10)
        self.title_frame.pack(fill=tk.X, pady=(0, 20))

        self.title_label = tk.Label(
            self.title_frame, text="Título", font=self.title_font, bg="#FFEF99", fg="#000000")
        self.title_label.pack()

        # Crear contenedor para los juegos y reportes
        self.games_container = tk.Frame(self.main_frame, bg="#FFB6B6")
        ...
        self.monitor_frame = tk.Frame(
            self.main_frame, bg="#E0E0E0", padx=5, pady=5)
        ...
        if not self.connected:
            ...
            if port == "No hay puertos disponibles" or port == "Seleccionar puerto":
                messagebox.showerror(
                    "Error", "Por favor selecciona un puerto válido")
                return
    ...
    def select_game(self, game_num):
        ...
        try:
            # Enviar comando al Arduino (formato: G1, G2, G3)
            ...
            messagebox.showerror("Error de Comunicación",
                                 f"Error al enviar comando: {str(e)}")
    ...
    def open_report(self, report_num):
        ...
        graph_frame = tk.Frame(report_frame, bg="#A7D5F2",
                               padx=10, pady=10)  # Fondo azul claro
        ...
        canvas = tk.Canvas(graph_frame, bg="#A7D5F2",
                           height=100, highlightthickness=0)
        canvas.pack(fill=tk.X)
        # Dibujar una línea simulando un gráfico
        canvas.create_line(50, 80, 150, 30, 250, 60, 350,
                           20, 450, 40, fill="blue", width=2)
