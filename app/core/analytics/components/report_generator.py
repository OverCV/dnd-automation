...
"""
        ...
        # Recomendaciones
        report += self._add_recommendations_section(events, error_events, duration_events, score_events)
...
"""
    ...
    def _add_recommendations_section(self, all_events: List[Dict], error_events: List[Dict], 
                                   duration_events: List[Dict], score_events: List[Dict]) -> str:
        ...
        if score_events and len(score_events) > 3:
            ...
            if score_variance > 0.7:
                recommendations += "• Scores muy variables: trabaja en la consistencia\n"
        ...
        if len(all_events) > 10:
            ...
            time_span = (max(timestamps) - min(timestamps)).total_seconds() / 3600  # horas
            if time_span > 1:
                recommendations += f"• Sesión de {time_span:.1f} horas: toma descansos regulares\n"
...
"""
        ...
        for game_name, events in games_data.items():
            ...
            durations = [e.get("game_duration", 0) for e in events if "game_duration" in e]
            if durations:
                total_time = sum(durations)
    ...
    def export_data_to_csv(self, game_name: str, events: List[Dict], output_path: str = None) -> str:
    ...
    def export_multiple_games_to_excel(self, games_data: Dict[str, List[Dict]], output_path: str = None) -> str:
    ...
    def generate_custom_report(self, game_name: str, events: List[Dict], 
                             report_type: str = "basic", **kwargs) -> str:
    ...
    def _generate_basic_report(self, game_name: str, events: List[Dict], **kwargs) -> str:
...
"""
    ...
    def _generate_detailed_report(self, game_name: str, events: List[Dict], **kwargs) -> str:
        """Generar reporte detallado"""
        return self.generate_performance_report(game_name, events)
    
    def _generate_error_report(self, game_name: str, events: List[Dict], **kwargs) -> str:
