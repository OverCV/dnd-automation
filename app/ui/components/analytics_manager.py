        ...
        try:
            ...
            if not available_games:
                messagebox.showwarning(
                    "Sin datos", "No se encontraron logs de juegos para analizar"
                )
                return
            ...
            buttons_frame = tk.Frame(
                analytics_window, bg=self.colors.BACKGROUND_PRIMARY
            )
            ...
            game_combo = ttk.Combobox(
                game_frame, textvariable=game_var, values=available_games, width=30
            )
            game_combo.pack(pady=5)

            detail_buttons_frame = tk.Frame(
                analytics_window, bg=self.colors.BACKGROUND_PRIMARY
            )
...
"""
    ...
    def _export_to_csv(self, analytics, game_name):
        ...
        try:
            ...
            messagebox.showinfo(
                "Exportación", f"Datos de {game_name} exportados exitosamente"
            )
        except Exception as e:
